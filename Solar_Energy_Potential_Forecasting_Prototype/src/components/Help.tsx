      ...
      <Card className="border-2 border-emerald-200 shadow-md">
        ...
        <CardContent className="space-y-6 text-sm text-gray-700 leading-relaxed">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-600">Two separate scores are shown — one per model — both clipped to [35, 99]:</p>
            <div className="space-y-2 text-xs">
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 space-y-1">
                <p className="font-semibold text-blue-800">Baseline Confidence — spatial proximity only</p>
                <p className="font-mono text-blue-700">coverage = 100 − dist_km × 2.5</p>
                <p className="text-blue-700">The baseline is the reference model; it has no comparison partner, so its confidence reflects only how well the selected location is represented by the training data (kd-tree distance to nearest training point).</p>
              </div>
              <div className="rounded-lg border border-orange-200 bg-orange-50 p-3 space-y-1">
                <p className="font-semibold text-orange-800">FI-AdaBoost Confidence — spatial + model agreement</p>
                ...
                <p className="text-orange-700">FI-AdaBoost is an optimisation over the baseline, so its confidence is additionally penalised when it diverges strongly from the reference prediction — a large irradiance gap lowers this score.</p>
              </div>
            </div>
          </div>
        </CardContent>
            ...
            {[
              {
                ...
                detail: "FI-AdaBoost's typical irradiance error is ~1,495 J/m²/day smaller than the baseline. Because SEP = irradiance × rooftop area, this directly improves solar energy potential estimates — a building with 100 m² rooftop would see its daily SEP estimate shift by ~0.415 kWh/day (~152 kWh/year) less error.",
              },
              {
              ...
              {
                label: 'Daily DM = −1.54 (p = 0.108) — Temporal domain',
