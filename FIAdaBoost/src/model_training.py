...
def load_dataset() -> pd.DataFrame:
    ...
    df["shading_factor_log"] = np.log1p(df["shading_factor"] * 100)
    ...
    lambda r: (
        ...
        * (1 - 0.03* r["shading_factor"])   # 👈 small shading influence
    ),
    axis=1
    )
    return df
...
def plot_standalone_feature_importance(fi_vals, feats):
    ...
    for bar, val in zip(bars, sorted_vals):
        plt.text(val + 0.005, bar.get_y() + bar.get_height()/2, f"{val*100:.4f}%",
...
def main():
    ...
    print("\nFI-AdaBoost Feature Importances:")
    for feature, importance in zip(FI_FEATURES, fi.feature_importances_):
        print(f"  {feature:<25}: {importance * 100:.4f}%")
