...
def compute_energy_kwh(nasa: dict, features: dict[str, float]) -> float:
    ghi     = float(nasa["ghi_kwh"])
    area    = float(features["rooftop_area_sq_m"])
    sei     = float(features["SEI_norm"])
    shading = float(features["shading_factor"])
    energy  = ghi * area * PV_EFFICIENCY * PERFORMANCE_RATIO * max(sei, 0.20) * (1 - shading)
...
def _get_orientation_and_azimuth(azimuth_deg: float) -> tuple[str, float]:
    if 135 <= azimuth_deg <= 225:
        orientation = "South"
        ...
        orientation = "North"
    return (orientation, azimuth_deg)
...
def _build_predict_response(
    ...
    orientation, azimuth = _get_orientation_and_azimuth(features["azimuth_deg"])
...
def predict(payload: PredictRequest) -> PredictResponse:
    ...
    solar_kwh = compute_energy_kwh(nasa, features)
...
def compare_models(payload: PredictRequest) -> CompareResponse:
    ...
    fi_kwh = compute_energy_kwh(nasa, features)
