...
class ModelContext:
    ...
    def confidence_scores(self, lat: float, lng: float, diff_kwh: float) -> tuple[float, float]:
...
def compare_models(payload: PredictRequest) -> CompareResponse:
    ...
    try:
        ...
        baseline_confidence, fi_confidence = ctx.confidence_scores(payload.lat, payload.lng, irradiance_diff_kwh_m2)
